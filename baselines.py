...
class CLIP_finetune(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.clip_model == 'ViT-B/32' or args.clip_model == 'RN101' or args.clip_model == 'RN101_ImageNet100':
            num_filters = 512
...
class RESNET_finetune(nn.Module):
    ...
    def __init__(self, args):
        ...
        elif args.resnet_model == "101_I100":
            backbone = Baseline.load_from_checkpoint('rn101_imagenet100.ckpt') 

        #grab the input dimension to the final layer
        num_filters = backbone.fc.in_features if not args.resnet_model == "101_I100" else backbone.encoder.classifier.in_features

        #define the feature extraction module
        layers = list(backbone.children())[:-1] #leave out the fc layer!
        self.feature_extractor = nn.Sequential(*layers) if not args.resnet_model == "101_I100" else backbone.encoder.feature_extractor
