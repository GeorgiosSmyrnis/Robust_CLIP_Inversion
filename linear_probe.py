...
class LinearProbe(LightningModule):
    ...
    def __init__(self, args):
        ...
        if self.hparams.encoder == "clip":
            ...
            self.extra_layer = saved_student.extra_layer
        ...
        self.extra_layer.eval()
        for param in self.extra_layer.parameters():
            param.requires_grad = False

        #This is the meat
        self.output = nn.Linear(100, self.hparams.num_classes)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            if self.hparams.encoder == "clip":
                noisy_embeddings = self.extra_layer(self.backbone(x.type(torch.float16)).float())
            elif self.hparams.encoder == "clean":
