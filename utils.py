...
class RandomForwardOp(object):
    ...
    def __call__(self, image):
        ...
        if isinstance(self.percent_missing, float):
            removed_percent = self.percent_missing
        else:
            removed_percent = np.random.uniform(self.percent_missing[0], self.percent_missing[1])
        flat_img = image.flatten(start_dim=1)
        if self.rand_gauss is None:
            self.rand_gauss = torch.randn(int(h*w*(1-removed_percent)), h*w, device=flat_img.device)
        rand_gauss = self.rand_gauss
        interm = flat_img @ rand_gauss.T
        out_img = interm @ rand_gauss
        out_img = (out_img - out_img.min())/(out_img.max()-out_img.min())
        out_img.requires_grad = False
        return out_img.view(3,h,w)
...
class ImageNetDistortTrainContrastive:
    ...
    def __init__(self, args, epoch=None):
        ...
        if args.distortion == "squaremask":
            ...
            distortion = transforms.GaussianBlur(kernel_size=args.kernel_size, sigma=args.sigma)
        elif args.distortion == "randomforwardop":
            distortion = RandomForwardOp(percent_missing=args.percent_missing)
